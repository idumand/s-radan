...
export type CoinSelectionMode = 'manual' | 'algorithm';
...
export interface Trade {
  ...
  diagnostic?: string;
  duration_sec?: number;
  pnl_source?: "binance" | "local" | "closed_record";
  mark_price?: number;
}

export interface TradeDiagnosticItem {
  id: string;
  category: 'STOP_LOSS' | 'PREMATURE_EXIT' | 'FEE_SLIPPAGE' | 'TAKE_PROFIT' | 'BREAKEVEN_LOCK' | 'TREND_REVERSAL';
  title: string;
  count: number;
  impactUsd: number;
  description: string;
...
export interface TradeAnalytics {
  ...
  longTrades: { count: number; winRatePct: number; netPnlUSD: number };
  shortTrades: { count: number; winRatePct: number; netPnlUSD: number };
  pairPerformance: Array<{ pair: string; trades: number; winRatePct: number; netPnlUSD: number }>;
  diagnostics: TradeDiagnosticItem[];
...
export interface MarketPairInfo {
  ...
  flowDirection?: 'STRONG_LONG' | 'LONG' | 'NEUTRAL' | 'SHORT' | 'STRONG_SHORT';
  obi?: number;
  deepScore?: number;
}
