        ...
        <div className="flex flex-wrap gap-2 pt-1 pb-2">
          {openTrades.map((trade) => (
            ...
            >
              ...
              {trade.pnl_source === 'binance' && (
                <span className="text-[9px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-300 border border-blue-500/20">BINANCE</span>
              )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-[#151921] border border-[#1e232f] p-3 rounded-xl flex flex-wrap items-center justify-between gap-3">
            ...
            {/* Mode Indicator */}
            {metrics.coin_selection_mode === 'algorithm' ? (
              <div className="flex items-center space-x-1.5 bg-cyan-500/10 border border-cyan-500/30 px-3 py-1.5 rounded-lg text-xs font-semibold text-cyan-300 shadow-sm">
                <Zap className="w-3.5 h-3.5 text-cyan-400 animate-pulse" />
                <span>Algoritma Modu Aktif (Piyasadaki En İyi Coinler Otomatik Seçilir)</span>
              </div>
            ) : (
              <div className="flex items-center space-x-1.5 bg-emerald-500/10 border border-emerald-500/30 px-3 py-1.5 rounded-lg text-xs font-semibold text-emerald-300 shadow-sm">
                <ListFilter className="w-3.5 h-3.5 text-emerald-400" />
                <span>Manuel Parite Modu Aktif (Sadece Seçtiğiniz Paritelerde İşlem Açar)</span>
              </div>
            )}
          </div>
        ...
        <div className="bg-[#151921] border border-[#1e232f] p-4 rounded-xl space-y-4 flex flex-col">
          <div className="flex items-center justify-between pb-2 border-b border-[#1e232f]">
            <h3 className="font-bold text-sm text-white flex items-center space-x-2">
              ...
              <span>{metrics.coin_selection_mode === 'algorithm' ? 'Algoritma Tarama Listesi' : 'Seçili Parite Listesi'}</span>
            </h3>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1 max-h-[480px]">
            {markets.map((m) => {
              ...
              const buyRatio = m.takerBuyRatio !== undefined ? Math.round(m.takerBuyRatio * 100) : 50;
              const longScore = Math.round(m.longFlowScore || 50);
              const shortScore = Math.round(m.shortFlowScore || 50);
              const netInflow = Math.round(m.netInflowUSD || 0);
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-bold text-xs font-mono text-white flex items-center space-x-1.5">
                        <span>{m.symbol}</span>
                        {m.signal === 'BUY' && (
                          <span className="text-[10px] font-bold bg-emerald-500/20 text-emerald-400 px-1.5 py-0.2 rounded font-sans flex items-center space-x-0.5">
                            <ArrowUpRight className="w-2.5 h-2.5" />
                            <span>LONG</span>
                          </span>
                        )}
                        {m.signal === 'SELL' && (
                          <span className="text-[10px] font-bold bg-rose-500/20 text-rose-400 px-1.5 py-0.2 rounded font-sans flex items-center space-x-0.5">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-[10px] text-slate-500 font-mono">
                        {netInflow !== 0 ? (
                          <span className={netInflow > 0 ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
                            {netInflow > 0 ? '+' : ''}${Math.round(netInflow / 1000)}k akış
                          </span>
                        ) : (
                  ...
                  {/* Long vs Short Money Flow Mini-Bar */}
                  <div className="space-y-1 pt-1 border-t border-slate-800/80">
                    <div className="flex justify-between items-center text-[10px] font-mono">
                      <span className={`font-semibold ${longScore >= shortScore ? 'text-emerald-400' : 'text-slate-400'}`}>
                        L: {longScore}
                      </span>
                      <span className="text-slate-500 text-[9px]">
                        %{buyRatio} Alıcı
                      </span>
                      <span className={`font-semibold ${shortScore > longScore ? 'text-rose-400' : 'text-slate-400'}`}>
                        S: {shortScore}
                      </span>
                    </div>
                    <div className="w-full h-1 bg-slate-800 rounded-full flex overflow-hidden">
                      <div className="h-full bg-emerald-500 transition-all duration-200" style={{ width: `${buyRatio}%` }} />
                      <div className="h-full bg-rose-500 transition-all duration-200" style={{ width: `${100 - buyRatio}%` }} />
                    </div>
                  </div>
                </div>
              );
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-800/30 transition">
                      ...
                      <td className="py-3 px-4">
                        ...
                        {t.pnl_source === 'binance' && (
                          <div className="text-[9px] text-blue-300 mt-0.5">Binance PnL / Mark Price</div>
                        )}
                      </td>
