...
import { Settings, Save, Search, X, CheckCircle2, AlertCircle, Zap, Shield, Target, Eye, EyeOff, Globe, Copy, Check, RefreshCw, Cpu, ListFilter, Sparkles } from 'lucide-react';
...
export const ConfigEditor: React.FC<ConfigEditorProps> = ({
  ...
  const [coinSelectionMode, setCoinSelectionMode] = useState<'manual' | 'algorithm'>('manual');
  const [stakeAmount, setStakeAmount] = useState<string>("6");
  const [leverage, setLeverage] = useState<string>("15");
  const [maxOpenTrades, setMaxOpenTrades] = useState<string>("1");
  const [minExpectedMovePct, setMinExpectedMovePct] = useState<string>("0.5");
  const [takeProfitPct, setTakeProfitPct] = useState<string>("0.5");
  ...
  useEffect(() => {
    try {
      if (initialConfigJson) {
        ...
        setParsedConfig(cfg);
        if (cfg.coin_selection_mode === 'algorithm' || cfg.coin_selection_mode === 'manual') {
          setCoinSelectionMode(cfg.coin_selection_mode);
        }
        ...
        if (cfg.min_expected_move_pct !== undefined && cfg.min_expected_move_pct !== null) {
          ...
          setMinExpectedMovePct("0.5");
        }
        if (cfg.take_profit_pct !== undefined && cfg.take_profit_pct !== null) {
          setTakeProfitPct(String(cfg.take_profit_pct));
        } else {
          setTakeProfitPct("0.5");
        }
        setMarginMode(String(cfg.margin_mode || 'isolated').toLowerCase() === 'cross' ? 'cross' : 'isolated');
      }
    } catch(e) {}
  }, [initialConfigJson]);
  ...
  const handleModeChange = (mode: 'manual' | 'algorithm') => {
    setCoinSelectionMode(mode);
    handleUpdate('coin_selection_mode', mode);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const finalStake = Math.min(Math.max(parseDecimal(stakeAmount, 6), 1), 1000000);
      const finalLeverage = Math.min(Math.max(Math.round(parseDecimal(leverage, 15)), 1), 125);
      const finalMaxOpenTrades = Math.min(Math.max(Math.round(parseDecimal(maxOpenTrades, 1)), 1), 8);
      const finalMinExpectedMovePct = Math.min(Math.max(parseDecimal(minExpectedMovePct, 0.5), 0.5), 20);
      const finalTakeProfitPct = Math.min(Math.max(parseDecimal(takeProfitPct, 0.5), 0.1), 20);
      const rawStop = parseDecimal(String(parsedConfig?.stop_loss_pct ?? ''), 1.0);
      const finalStopLossPct = Math.min(Math.max(Math.abs(rawStop), 0.1), 20);
      ...
      const updated = {
        ...
        take_profit_pct: finalTakeProfitPct,
      ...
      setTakeProfitPct(String(updated.take_profit_pct));
      setParsedConfig(updated);

      await onSaveConfig(JSON.stringify(updated, null, 2));
      setSuccess("Ayarlar başarıyla kaydedildi. Ondalık değerler aynen korunur.");
  ...
  const handleResetDefaults = async () => {
    ...
    try {
      await onSaveConfig(JSON.stringify(updated, null, 2));
      setSuccess('Güvenli varsayılan ayarlar geri yüklendi.');
      setTimeout(() => setSuccess(null), 3000);
    } catch (e: any) {
      setError(e.message);
    }
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center pb-2 border-b border-[#1e232f]">
        <h2 className="text-lg font-bold flex items-center space-x-2 text-white">
          ...
          <span>Sistem & Algoritma Yapılandırma Ayarları</span>
        </h2>
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={handleResetDefaults}
            className="flex items-center space-x-1.5 bg-slate-700/60 hover:bg-slate-600 text-slate-200 font-semibold px-3 py-2 rounded-lg text-xs transition"
          >
      ...
      {/* COIN SELECTION MODE SELECTOR CARD */}
      <div className="bg-[#151921] border border-[#1e232f] p-5 rounded-xl shadow-xl space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-[#1e232f]">
          <div>
            <h3 className="font-bold text-sm text-white flex items-center space-x-2">
              <ListFilter className="w-4 h-4 text-emerald-400" />
              <span>Parite & Coin Seçim Modu</span>
            </h3>
            <p className="text-xs text-slate-400 mt-0.5">
              Botun işlem açacağı coinleri manuel mi yoksa algoritmanın otomatik mi seçeceğini belirleyin.
            </p>
          </div>

          {/* Mode Switcher Segmented Buttons */}
          <div className="flex items-center bg-[#0b0e14] p-1 rounded-xl border border-slate-800 shrink-0">
            <button
              type="button"
            ...
            >
              ...
              <span>Algoritma Modu</span>
            </button>
          </div>
        </div>

        {/* TAB 1: MANUEL SEÇİM GÖRÜNÜMÜ */}
        {coinSelectionMode === 'manual' && (
          <div className="space-y-4">
            <div className="bg-emerald-500/10 border border-emerald-500/30 p-3.5 rounded-lg text-xs text-emerald-300 flex items-start space-x-2.5">
              <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
              <div>
                <span className="font-bold">Manuel Parite Modu Aktif:</span> Bot sadece aşağıda eklediğiniz listedeki <span className="font-mono font-bold text-white">({currentCoins.length} parite)</span> üzerinde analiz yapar ve yalnızca bu coinlerde pozisyon açar.
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              <span className="text-[11px] text-slate-400 block font-medium">Hızlı Parite Ekle:</span>
            ...
            <div className="pt-2 border-t border-[#1e232f]">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-semibold text-slate-300">Botun İşlem Açacağı Seçili Pariteler ({currentCoins.length}):</span>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                {currentCoins.length === 0 && (
                  <span className="text-amber-400/80 text-xs italic bg-amber-500/10 px-3 py-1.5 rounded-lg border border-amber-500/20">
                    Henüz parite seçilmedi. Arama kutusundan arayarak veya hızlı butonlardan en az 1 parite ekleyin.
                  </span>
                )}
              </div>
            </div>
        ...
        {/* TAB 2: ALGORİTMA / DİNAMİK TARAMA GÖRÜNÜMÜ */}
        {coinSelectionMode === 'algorithm' && (
          <div className="space-y-4">
            <div className="bg-cyan-500/10 border border-cyan-500/30 p-4 rounded-xl text-xs text-cyan-300 space-y-2">
              <div className="flex items-center space-x-2 font-bold text-sm text-cyan-200">
                <Sparkles className="w-4 h-4 text-cyan-400 animate-pulse" />
                <span>Otomatik Algoritmik Tarama Modu Aktif</span>
              </div>
              <p className="text-slate-300 leading-relaxed">
                Bu mod seçildiğinde bot, Binance Vadeli (Futures) piyasasındaki en yüksek hacimli, likit ve volatil 30+ coini sürekli tarar. Anlık <span className="text-cyan-400 font-semibold">Taker Para Akışı</span>, <span className="text-cyan-400 font-semibold">Order Book Dengesizliği (OBI)</span> ve <span className="text-cyan-400 font-semibold">Direnç Duvarı</span> puanlaması yaparak en yüksek başarı olasılığına sahip coinde otomatik pozisyon açar.
              </p>
            </div>

            {/* Live Monitored Top Algorithm Coins Preview */}
            <div className="p-4 bg-[#0b0e14] rounded-xl border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-300 flex items-center space-x-1.5">
                  <Cpu className="w-4 h-4 text-cyan-400" />
                  <span>Algoritmanın Taramaya Aldığı Yüksek Hacimli Pariteler (30+)</span>
                </span>
                <span className="text-[10px] bg-cyan-500/20 text-cyan-300 px-2 py-0.5 rounded font-mono font-semibold">
                  Otomatik Taranıyor
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {[
                  ...
                  "ADA/USDT", "ARB/USDT", "OP/USDT", "FTM/USDT", "TRX/USDT", "BCH/USDT", "SEI/USDT", "BONK/USDT"
                ].map((p) => (
                  <span
                    key={p}
                    className="text-[11px] font-mono px-2.5 py-1 rounded-lg bg-[#151921] border border-cyan-500/20 text-slate-300 flex items-center space-x-1"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-2 text-[11px] text-slate-400">
                <div className="bg-[#151921] p-2 rounded-lg border border-slate-800">
                  <div className="font-semibold text-white mb-0.5">1. Para Akışı Taraması</div>
                  <div>Taker alıcı ve satıcı net USD akış hacimleri anlık ölçülür.</div>
                </div>
                <div className="bg-[#151921] p-2 rounded-lg border border-slate-800">
                  <div className="font-semibold text-white mb-0.5">2. Direnç Analizi</div>
                  <div>Emir defterindeki duvarlar taranır, önü açık pariteler seçilir.</div>
                </div>
                <div className="bg-[#151921] p-2 rounded-lg border border-slate-800">
                  <div className="font-semibold text-white mb-0.5">3. Otomatik Seçim</div>
                  <div>En yüksek sinyal puanına sahip coinde pozisyon açılır.</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#151921] border border-[#1e232f] p-5 rounded-xl shadow-xl space-y-4">
            ...
            <div>
               <label className="block text-xs font-semibold text-slate-300 mb-1.5">Marjin Modu</label>
               <select
                 value={marginMode}
                 onChange={(e) => {
                   const mode = e.target.value === 'cross' ? 'cross' : 'isolated';
                   setMarginMode(mode);
                   handleUpdate('margin_mode', mode);
                 }}
                 className="w-full bg-[#0b0e14] border border-slate-700 text-white text-sm rounded-lg p-3 focus:outline-none focus:border-emerald-500 transition-colors"
               >
                 <option value="isolated">ISOLATED — Her pozisyon ayrı marjin</option>
                 <option value="cross">CROSS — Ortak cüzdan marjini</option>
               </select>
               <p className="text-[10px] text-slate-500 mt-1">Emir açılırken seçtiğiniz Binance Futures marjin modu uygulanır. Varsayılan güvenli seçenek ISOLATED'dır.</p>
            </div>

            <div>
               <div className="flex items-center justify-between mb-1.5">
                 <label className="text-xs font-semibold text-slate-300">Minimum Beklenen Hareket / Kâr Hedefi (1x)</label>
                 <span className="text-[11px] font-mono text-cyan-400 font-bold">%{minExpectedMovePct || '0.5'}</span>
               </div>
               <input
                  type="number"
                  min="0.5"
               ...
               />
               <p className="text-[10px] text-slate-500 mt-1">Giriş filtresidir. %0,5 gibi ondalık değerler desteklenir; kaydedildiğinde yuvarlanmaz.</p>
            </div>

            <div>
               <div className="flex items-center justify-between mb-1.5">
                 <label className="text-xs font-semibold text-slate-300">Manuel Kâr Hedefi (TP, 1x fiyat hareketi)</label>
                 <span className="text-[11px] font-mono text-emerald-400 font-bold">%{takeProfitPct || '0.5'}</span>
               </div>
               <input
                  type="number"
                  min="0.1"
                  ...
                  onChange={(e) => {
                    ...
                    const num = parseFloat(val.replace(',', '.'));
                    if (!isNaN(num)) handleUpdate("take_profit_pct", num);
                  }}
                  placeholder="Örn: 0.5"
                  className="w-full bg-[#0b0e14] border border-slate-700 text-white font-mono text-sm rounded-lg p-3 focus:outline-none focus:border-emerald-500 transition-colors"
               />
               <p className="text-[10px] text-slate-500 mt-1">Pozisyon açıldıktan sonra fiyat giriş yönünde bu yüzdeye ulaştığında manuel TP devreye girer. 15x kaldıraçta %0,5 fiyat hareketi yaklaşık %7,5 ROE'ye denk gelir (ücretler hariç).</p>
            </div>
          </div>
          ...
          <div className="bg-[#151921] border border-[#1e232f] p-5 rounded-xl shadow-xl space-y-4">
            ...
            <div>
               ...
               <input 
                  ...
                  min="0.1"
                  step="0.1"
                  value={parsedConfig?.stop_loss_pct !== undefined ? parsedConfig.stop_loss_pct : 1.0}
