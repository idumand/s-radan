        ...
        <div className="space-y-3">
          ...
          <div className="bg-[#0b0e14] p-3 border border-[#2a3142] rounded-lg">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] uppercase font-bold text-slate-400">Algoritmik Para Akışı Puanlaması (Long / Short)</span>
              <span className={`text-xs font-mono font-bold ${
                flowDirection.includes('LONG') ? 'text-emerald-400' : flowDirection.includes('SHORT') ? 'text-rose-400' : 'text-slate-400'
              }`}>
                {flowDirection === 'STRONG_LONG' ? 'GÜÇLÜ LONG' : flowDirection === 'LONG' ? 'LONG SİNYALİ' : flowDirection === 'STRONG_SHORT' ? 'GÜÇLÜ SHORT' : flowDirection === 'SHORT' ? 'SHORT SİNYALİ' : 'NÖTR / DENGELİ'}
              </span>
            </div>
            <div className="flex items-center justify-between text-xs font-mono mb-1">
              <span className="text-emerald-400 font-bold">Long Skoru: {Math.round(longFlowScore)}/100</span>
              <span className="text-rose-400 font-bold">Short Skoru: {Math.round(shortFlowScore)}/100</span>
            </div>
            <div className="w-full h-2 bg-slate-800 rounded-full flex overflow-hidden">
              <div className="h-full bg-emerald-500 transition-all duration-200" style={{ width: `${longFlowScore}%` }} />
              <div className="h-full bg-rose-500 transition-all duration-200" style={{ width: `${shortFlowScore}%` }} />
            </div>
          </div>
