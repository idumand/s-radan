...
export function App() {
        ...
        if (data && Object.keys(data).length > 0) {
          ...
          if (data.stake_amount) {
            setMetrics(prev => ({ 
              ...
              coin_selection_mode: data.coin_selection_mode || 'manual'
            }));
          } else if (data.coin_selection_mode) {
            setMetrics(prev => ({
              ...prev,
              coin_selection_mode: data.coin_selection_mode
            }));
          }
        }
      } catch (e) {}
  ...
  useEffect(() => {
    const fetchEngineStatus = async () => {
      try {
        ...
        if (statusData.coin_selection_mode) {
          setMetrics(prev => ({
            ...prev,
            coin_selection_mode: statusData.coin_selection_mode
          }));
        }
  ...
  // Market data comes exclusively from the backend gateway.
  // The browser must never connect directly to Binance, otherwise TESTNET can
  // accidentally mix with LIVE market data.
  useEffect(() => {
    ...
    const processTickData = (items: any[]) => {
      ...
      setTrades((prevTrades) => {
        ...
        const newTrades = prevTrades.map((t) => {
          ...
          if (tickMap.has(binanceSymbol) && t.pnl_source !== 'binance') {
            const tick = tickMap.get(binanceSymbol);
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (deepData && deepData.metrics) {
          setMarkets((prevMarkets) => {
            return prevMarkets.map((m) => {
              const metric = deepData.metrics[m.symbol];
              if (!metric) return m;
              const longFlowScore = metric.longFlowScore !== undefined ? metric.longFlowScore : (metric.longAdvantage || 50);
              const shortFlowScore = metric.shortFlowScore !== undefined ? metric.shortFlowScore : (metric.shortAdvantage || 50);
              const flowDirection = metric.flowDirection || (longFlowScore > shortFlowScore + 2 ? 'LONG' : shortFlowScore > longFlowScore + 2 ? 'SHORT' : 'NEUTRAL');
              const signal = flowDirection === 'STRONG_LONG' || flowDirection === 'LONG' ? 'BUY' : (flowDirection === 'STRONG_SHORT' || flowDirection === 'SHORT' ? 'SELL' : 'NEUTRAL');

              return {
                ...m,
                price: metric.currentPrice || m.price,
                change_24h_pct: metric.change_24h_pct !== undefined ? Number(metric.change_24h_pct.toFixed(2)) : m.change_24h_pct,
                volume_24h_usdt: metric.volume_24h_usdt || m.volume_24h_usdt,
                signal,
                takerBuyRatio: metric.takerBuyRatio !== undefined ? metric.takerBuyRatio : m.takerBuyRatio,
                takerSellRatio: metric.takerSellRatio !== undefined ? metric.takerSellRatio : m.takerSellRatio,
                netInflowUSD: metric.netInflowUSD !== undefined ? metric.netInflowUSD : m.netInflowUSD,
                longFlowScore,
                shortFlowScore,
                flowDirection,
                obi: metric.obi !== undefined ? metric.obi : m.obi,
                deepScore: metric.deepScore !== undefined ? metric.deepScore : m.deepScore
              };
            });
          });
        }
  ...
  return (
    <div className="min-h-screen bg-[#0b0e14] text-slate-100 flex flex-col">
      ...
      <BinanceSettingsModal
        ...
        initialApiKey={(() => {
          try {
            return JSON.parse(configJson)?.exchange?.key || '';
          } catch {
            return '';
          }
        })()}
        initialSecretKey={(() => {
          try {
            return JSON.parse(configJson)?.exchange?.secret || '';
          } catch {
            return '';
          }
        })()}
        initialEnvironment={(() => {
          try {
            return JSON.parse(configJson)?.exchange?.environment === 'testnet' ? 'testnet' : 'live';
          } catch {
            return 'testnet';
          }
        })()}
        serverIp={serverIp}
        onSave={async (key, secret, env) => {
          try {
            let parsed: any = {};
            try {
            ...
            parsed.exchange.key = key;
            parsed.exchange.secret = secret;
            parsed.exchange.environment = env;
            const newJsonStr = JSON.stringify(parsed, null, 2);
            setConfigJson(newJsonStr);
            ...
            const res = await fetch('/api/v1/exchange-keys', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ apiKey: key, secretKey: secret, environment: env }),
            });
            const data = await res.json();
            ...
            if (data.success) {
              ...
              addLog(
                'SYSTEM',
                `Binance ${env === 'testnet' ? 'TESTNET' : 'CANLI'} Vadeli Cüzdan bağlandı! Bakiye: $${
                  data.balance_usdt !== undefined ? Number(data.balance_usdt).toFixed(2) : '0.00'
                } USDT`
              );
              return { success: true, balance_usdt: data.balance_usdt };
            } else {
              const errMsg = data.message || 'Bilinmeyen Hata';
              addLog('ERROR', `Binance API bağlantı hatası: ${errMsg}`);
