...
async function syncLocalBook(symbol: string, retries = 2) {
  ...
  try {
    ...
    try {
      r = await fetch(`${futuresRestBase()}/fapi/v1/depth?symbol=${encodeURIComponent(clean)}&limit=100`, {
...
function futuresRestBase() {
  return getBinanceEnvironment() === "testnet" ? "https://demo-fapi.binance.com" : "https://fapi.binance.com";
}

function futuresWsBase() {
  // Binance USD-M Futures Demo/Test market-data streams use the Futures stream host.
  return getBinanceEnvironment() === "testnet" ? "wss://fstream.binancefuture.com" : "wss://fstream.binance.com";
}
...
async function initializeExchange() {
  try {
    ...
    if (isTestnet) {
      ...
      } else if (typeof (exchange as any).setSandboxMode === "function") {
        try {
    ...
    await syncBinanceTimeOffset();
    if (typeof (exchange as any).loadTimeDifference === "function") {
      try {
        await (exchange as any).loadTimeDifference();
      } catch (e) {}
    }
...
function startBinanceServerWebSocket() {
  ...
  const previousClient = binanceWsClient;
  binanceWsClient = null;
  if (previousClient) {
    try { previousClient.removeAllListeners(); } catch {}
    try { previousClient.terminate(); } catch (e) {}
  }
  try {
    const activePairs = getActiveTradingPairs();
    const streamNamesFutures = activePairs
    ...
    try {
      ...
      const client = new WsClient(urlFutures);
      binanceWsClient = client;

      client.on('open', () => {
        const modeLabel = coinSelectionMode === 'algorithm' ? 'Algoritma Modu (Dinamik En İyi Coinler)' : 'Manuel Mod (Seçili Pariteler)';
        addEngineLog("INFO", `Binance Vadeli (Futures) 100ms WebSocket akışına bağlanıldı [${modeLabel}: ${activePairs.length} parite].`);
      ...
      client.on('error', () => {});
      client.on('close', () => {
        if (binanceWsClient !== client) return;
        clearTimeout(binanceWsReconnectTimer);
    ...
    activePairs.map(async (symbol) => {
      try {
        ...
        if (!recentTradesMap[symbol] || recentTradesMap[symbol].length < 10) {
          try {
            const tradesRes = await fetch(`${futuresRestBase()}/fapi/v1/trades?symbol=${cleanSymbol}&limit=50`);
            if (tradesRes.ok) {
...
app.get("/api/v1/live-tickers", (req, res) => {
  ...
  res.json({ tickers: results, coin_selection_mode: coinSelectionMode });
});
...
async function syncBinanceTimeOffset(): Promise<number> {
  const now = Date.now();
  if (now - lastTimeSync < 60000 && lastTimeSync !== 0) {
    return binanceServerTimeOffset;
  }
  const isTestnet = getBinanceEnvironment() === "testnet";
...
async function fetchDirectBinanceFuturesBalance(
  ...
  const offset = await syncBinanceTimeOffset();
...
app.post("/api/v1/exchange-keys", async (req, res) => {
  ...
  if (!cleanKey || !cleanSecret) {
    ...
    await initializeExchange();
    return res.json({ success: true, balance_usdt: 0, environment: targetEnv });
  }
  ...
  if (!validationSuccess) {
    try {
      ...
      if (targetEnv === "testnet") {
        ...
        } else if (typeof (tempExchange as any).setSandboxMode === "function") {
          try {
